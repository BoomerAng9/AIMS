...
Date: 2026-09-24
Version: 1.3 (verifies Paperclip's documented external-adapter install API and alpha caveat; does not broaden implementation or deployment authority)
...
- Official Paperclip documentation reviewed 2026-09-24 documents external adapter installation through the instance-admin `POST /api/adapters/install` endpoint and readback through `GET /api/adapters`; it also labels external adapters alpha and recommends pinning versions. This confirms a supported integration mechanism in the documented product, not availability in the owner's deployed Paperclip instance. The next no-write runtime check is an authenticated, read-only adapter inventory against the actual Paperclip API, once its established endpoint/access path is available. [External adapter guide](https://docs.paperclip.ing/reference/adapters/external-adapters/) · [Adapters API](https://docs.paperclip.ing/reference/api/adapters/) · [Adapter Manager](https://docs.paperclip.ing/guides/org/adapters/)
