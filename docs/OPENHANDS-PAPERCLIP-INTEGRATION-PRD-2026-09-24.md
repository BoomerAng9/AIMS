...
Date: 2026-09-24
Version: 1.1 (adds read-only live runtime and Spinner/JEV evidence; does not broaden deployment authority)
...
- The protected Spinner MCP registry, read using its bridge credential without returning the secret, lists five tools: `build_site`, `deploy_agent`, `find_route`, `geocode_place`, and `agent_task`; it exposes no JEV decision tool. Spinner's existing `JEV_API_KEY` binding successfully authenticated a GET to TypeSafe `/v1/models`, which returned `jev-latest`. No inference request was sent and no model behavior, policy, or cost was inferred from the key/model-list response. TypeSafe's documented endpoint is `POST https://api.typesafe.ai/v1/systemone`; it accepts a typed state/questions request and returns structured answers and usage. [Official API reference](https://docs.typesafe.ai/api)
