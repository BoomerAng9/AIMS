...
Date: 2026-09-24
Version: 1.6 (adds a sanitized, authenticated adapter inventory from the deployed Paperclip instance; does not broaden implementation or deployment authority)
...
- Read-only live checks through Spinner to the configured Paperclip origin returned `GET /api/health` = 200 (`status: ok`, `deploymentMode: authenticated`, commit `65ec059bde30d98c92165b24a30a540800dd1f6f`), matching upstream release tag `v2026.831.1`. `GET /api/agents/me` identified the existing bearer as agent-scoped LUC; it returned 401 on board-only `GET /api/cli-auth/me` and 403 on `GET /api/adapters`. Separately, an existing board-admin sign-in flow using the already-stored Spinner-side admin credentials established an authenticated session; `GET /api/adapters` returned 200 with 16 adapter types: `acpx_local`, `claude_local`, `codex_local`, `cursor`, `cursor_cloud`, `gemini_local`, `grok_local`, `hermes_gateway`, `hermes_local`, `http`, `kimi_local`, `openclaw_gateway`, `opencode_local`, `paperclip_runner`, `pi_local`, and `process`. The session cookie and credential values were not surfaced or persisted. None of the returned built-in types is explicitly an OpenHands/Agent Canvas adapter; the generic `http` type is not evidence of a supported Canvas execution contract. No adapter installation, config change, assignment, or task execution occurred. [Authentication reference](https://docs.paperclip.ing/reference/api/authentication/) · [Board/agent token guide](https://docs.paperclip.ing/reference/cli/authentication/) · [Upstream commit](https://github.com/paperclipai/paperclip/commit/65ec059bde30d98c92165b24a30a540800dd1f6f) · [Release tag](https://github.com/paperclipai/paperclip/tree/v2026.831.1)
