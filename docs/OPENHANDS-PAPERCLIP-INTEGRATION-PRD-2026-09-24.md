...
Date: 2026-09-24
Version: 1.5 (maps the live Paperclip build commit to its official release tag; does not broaden implementation or deployment authority)
...
Unknown: deployed Paperclip version and adapter-plugin enablement; canonical Paperclip API endpoint and credential scope; Agent Server API authentication/key-rotation policy and live conversation/event/file/cancel contract; approved Second Brain tenant resolver and OKF revision source; per-run budget/cost reporting; exact durable idempotency store across gateway replicas; selected internal workspace allowlist; operator's authenticated Paperclip/Canvas verification surface; production owner approval and rollback procedure. One bounded JEV System One request has been tested directly; integration of that decision into an A.I.M.S. broker or routed task remains unimplemented and unverified.
...
- Official Paperclip documentation reviewed 2026-09-24 documents external adapter installation through the instance-admin `POST /api/adapters/install` endpoint and readback through `GET /api/adapters`; it also labels external adapters alpha and recommends pinning versions. The live Paperclip URL is now resolved from the existing Spinner runtime configuration, but only its public health and agent-scoped identity have been verified; the board-scoped adapter inventory remains unavailable with that agent key. [External adapter guide](https://docs.paperclip.ing/reference/adapters/external-adapters/) · [Adapters API](https://docs.paperclip.ing/reference/api/adapters/) · [Adapter Manager](https://docs.paperclip.ing/guides/org/adapters/)
- Read-only requests through the existing Spinner runtime to the configured Paperclip origin returned `GET /api/health` = 200 (`status: ok`, `deploymentMode: authenticated`, commit `65ec059bde30d98c92165b24a30a540800dd1f6f`; the health response itself omitted a version). The exact commit matches upstream tag `v2026.831.1`, verifying the deployed Paperclip build version. `GET /api/agents/me` returned 200 and identified the credential as agent-scoped (agent name LUC; ID intentionally omitted). The same bearer credential returned 401 on board-only `GET /api/cli-auth/me` and 403 on instance-admin `GET /api/adapters`. No credential value was output, changed, or elevated. Official Paperclip auth docs distinguish agent keys from board keys and explicitly say agent keys cannot be widened into board authority. The exact next step is read-only adapter inventory using an already-authorized board-scoped credential from the established secret store or owner-approved board session; do not reuse or elevate the agent key. [Authentication reference](https://docs.paperclip.ing/reference/api/authentication/) · [Board/agent token guide](https://docs.paperclip.ing/reference/cli/authentication/) · [Upstream commit](https://github.com/paperclipai/paperclip/commit/65ec059bde30d98c92165b24a30a540800dd1f6f) · [Release tag](https://github.com/paperclipai/paperclip/tree/v2026.831.1)
...
|---|---|---|---|---|
| Paperclip external adapter plugin | Integrate | Official contract supports separately packaged adapters; the instance API documents `POST /api/adapters/install`, `GET /api/adapters`, reload, and config-schema routes. [External adapter guide](https://docs.paperclip.ing/reference/adapters/external-adapters/) · [Adapters API](https://docs.paperclip.ing/reference/api/adapters/) | Paperclip repository LICENSE reports MIT; use published interfaces and retain required notices if distributing copied portions. | Official docs label external-adapter runtime alpha and recommend pinning; live build is verified as `v2026.831.1`, but board-scoped API access, adapter inventory, and plugin enablement remain unverified. |
