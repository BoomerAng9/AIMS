        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {MERCH_ITEMS.map((item, i) => (
            ...
            >
              ...
              <div className="w-full aspect-square rounded-xl bg-white border border-wireframe-stroke mb-4 flex items-center justify-center">
                <span className="text-4xl opacity-30">
                  {MERCH_CATEGORIES.find(c => c.id === item.category)?.icon || '🛍'}
                </span>
              </div>

              <h3 className="text-sm font-bold text-slate-800 mb-1 group-hover:text-gold transition-colors">
                {item.name}
              </h3>
        ...
        <div className="text-center mt-16 max-w-md mx-auto">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-lg bg-slate-100/60 border border-wireframe-stroke text-slate-800 text-sm placeholder:text-slate-300 outline-none focus:border-gold/50"
            />
