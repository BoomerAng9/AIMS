...
export default function TransferPortalPage() {
    ...
    return (
        <div className="min-h-screen bg-white text-slate-800 selection:bg-cyan-400/30 pb-24">
            {/* Top Nav */}
            <nav className="border-b border-slate-100 bg-white/80 backdrop-blur-xl sticky top-0 z-50">
                <div className="max-w-[1400px] mx-auto px-6 h-16 flex items-center justify-between">
                    <Link href="/sandbox/perform" className="flex items-center gap-2 text-[0.65rem] text-slate-400 hover:text-cyan-400 transition-colors font-mono uppercase tracking-widest">
            ...
            >
                ...
                <motion.div variants={staggerItem} className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pb-6 border-b border-slate-200">
                    ...
                    <div className="flex gap-3 flex-wrap">
                        <div className="px-4 py-3 bg-cyan-400/5 border border-cyan-400/20 rounded-xl text-center min-w-[100px]">
                            ...
                            <div className="text-[0.55rem] font-mono text-slate-400 uppercase tracking-widest">Indexed</div>
                        </div>
                        <div className="px-4 py-3 bg-gold/5 border border-gold/20 rounded-xl text-center min-w-[100px]">
                            ...
                            <div className="text-[0.55rem] font-mono text-slate-400 uppercase tracking-widest">Avg P.A.I.</div>
                        </div>
                    </div>
                </motion.div>
                        ...
                        {POSITIONS.map(pos => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-lg text-[0.6rem] font-mono uppercase tracking-widest transition-colors whitespace-nowrap ${posFilter === pos
                                        ...
                                        : 'bg-slate-50 text-slate-400 hover:text-slate-800 hover:bg-slate-100 border border-slate-100'
                                    }`}
                            >
                                {pos}
                            </button>
                ...
                {loading && (
                    <motion.div variants={staggerItem} className="flex flex-col items-center justify-center py-20 gap-3">
                        ...
                        <span className="text-xs font-mono text-slate-400">Querying portal database...</span>
                    </motion.div>
                )}
                    ...
                    <motion.div variants={staggerItem} className="bg-[#0f0f0f] border border-cyan-400/10 rounded-2xl p-16 text-center">
                        ...
                        <div className="flex gap-3 justify-center">
                            ...
                            <Link
                                ...
                                className="px-6 py-3 bg-slate-50 border border-slate-200 text-slate-600 hover:text-slate-800 hover:bg-slate-100 text-xs font-mono font-bold uppercase tracking-widest rounded-lg transition-all"
                            >
                                ...
                                <div key={player.id} className="bg-[#0f0f0f] border border-slate-100 rounded-2xl overflow-hidden hover:border-cyan-400/20 transition-all shadow-xl">
                                    ...
                                    >
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-3 mb-1">
                                                <h3 className="text-xl font-display font-bold text-slate-800 truncate">{player.name}</h3>
                                        ...
                                        <div className="flex items-center gap-6">
                                            <div className="text-center">
                                                ...
                                                <div className="text-[0.5rem] font-mono text-slate-400 uppercase tracking-widest">P.A.I.</div>
                                            </div>
                                            <div className="text-center">
                                                <div className="text-2xl font-display font-black text-green-400">{player.nilValue || '—'}</div>
                                                <div className="text-[0.5rem] font-mono text-slate-400 uppercase tracking-widest">NIL Est.</div>
                                            </div>
                                    ...
                                    {isExpanded && (
                                        <div className="border-t border-slate-100 bg-white p-6 space-y-4">
                                            {player.scoutMemo && (
                                                <p className="text-sm text-slate-500 italic font-serif leading-relaxed">&ldquo;{player.scoutMemo}&rdquo;</p>
                                            )}
                                            {player.tags && player.tags.length > 0 && (
                                                <div className="flex flex-wrap gap-1.5">
                                                    {player.tags.map((tag: string, idx: number) => (
                                                        <span key={idx} className="px-2 py-0.5 bg-slate-50 border border-slate-200 rounded text-[0.55rem] font-mono text-slate-400">
                                                            {tag}
                                                        </span>
                                                    ))}
                                                </div>
                ...
                <motion.div variants={staggerItem} className="bg-cyan-400/5 border border-cyan-400/20 rounded-2xl p-10 text-center relative overflow-hidden">
                    ...
                    <h2 className="text-3xl md:text-4xl font-display font-black text-slate-800 tracking-tight mb-3">
                        Every roster gap is an <span className="text-cyan-400">opportunity</span>.
                    </h2>
                    <p className="text-sm text-slate-500 max-w-lg mx-auto mb-6 font-sans">
