  ...
  return (
    ...
    >
      ...
      <motion.header
        ...
        className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-4 border-b border-white/5"
      >
        <div>
        ...
        {/* View toggle */}
        <div className="flex items-center gap-1 p-1 rounded-lg border border-wireframe-stroke bg-black/40">
          <button
      ...
      {/* Content */}
      {viewMode === "map" ? (
        <motion.div variants={staggerItem} className="flex-1 mt-4 rounded-xl border border-wireframe-stroke bg-black/30 overflow-hidden relative">
          <PlatformMindMap />
        </motion.div>
      ) : (
        <motion.div
          ...
          {SECTIONS.map((section) => (
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-white/20 font-mono">{section.items.length}</span>
              </div>
                  ...
                  item.href ? (
                    ...
                    >
                      <span className={`w-1.5 h-1.5 rounded-full ${
                        item.status === "live" ? "bg-emerald-400" : "bg-white/20"
                      }`} />
                      <span className="flex-1 text-white/70 group-hover:text-white transition-colors">
                        {item.label}
                      </span>
                      <ArrowRight size={12} className="text-white/0 group-hover:text-gold/60 transition-all" />
                    </Link>
                  ) : (
                    <div
                      key={item.label}
                    ...
                    >
                      ...
                      <span className="text-[10px] font-mono text-white/20">PLANNED</span>
                    </div>
                  )
                ))}
              </div>
