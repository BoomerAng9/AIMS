            ...
            <button
              ...
              className={`px-4 py-2 text-sm rounded-lg border transition-colors flex items-center gap-2 ${
                engine === tab.id
                  ...
                  : 'text-zinc-400 border-white/10 hover:border-white/15'
              }`}
            >
              <span>{tab.emoji}</span>
              {tab.label}
            ...
            <div className="flex gap-2 mb-4">
              <button
                ...
                className={`px-3 py-1.5 text-xs rounded-lg border transition-colors ${
                  crowdView === 'submit' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'text-zinc-400 border-white/10'
                }`}
              >
                New Simulation
              </button>
              <button
                ...
                className={`px-3 py-1.5 text-xs rounded-lg border transition-colors ${
                  crowdView === 'history' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'text-zinc-400 border-white/10'
                }`}
              >
            ...
            {crowdView === 'submit' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-zinc-400 mb-1 block">Product / Idea Name</label>
                    <input
                      value={productName}
                      onChange={e => setProductName(e.target.value)}
                      placeholder="e.g., FitCheck — AI Style Advisor"
                      className="w-full px-3 py-2 bg-[#1F1F23]/60 border border-white/10 rounded-lg text-sm text-zinc-100 outline-none focus:border-blue-500/30"
                    />
                  </div>
                  <div>
                    <label className="text-xs text-zinc-400 mb-1 block">Target Demographic</label>
                    <input
                      value={targetDemo}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full h-48 bg-[#1F1F23]/60 border border-white/10 rounded-lg p-3 text-sm text-zinc-100 font-mono outline-none focus:border-blue-500/30 resize-y"
                  />
                </div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto space-y-4">
                ...
                <div className="bg-[#111113] border border-white/10 shadow-[0_1px_2px_rgba(0,0,0,0.3)] rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-xs font-mono text-zinc-400">THE CROWD — LIVE FEED</p>
                  </div>
                  <div ref={eventLogRef} className="h-64 overflow-y-auto space-y-1 font-mono text-xs">
                    ...
                    {events.length === 0 && (
                      <p className="text-zinc-400">Assembling The Crowd...</p>
                    )}
                  </div>
                </div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto">
                {history.length === 0 ? (
                  <div className="text-center py-20">
                    <p className="text-zinc-400 text-lg">No simulations yet</p>
                    <p className="text-zinc-400 text-sm mt-1">Run your first Crowd simulation to see results here</p>
                  </div>
                ) : (
                    ...
                    {history.map(r => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full text-left bg-[#111113] border border-white/10 shadow-[0_1px_2px_rgba(0,0,0,0.3)] rounded-xl p-4 hover:border-white/15 transition-colors"
                        >
                          ...
                          <div className="flex gap-4 mt-2 text-xs">
                            ...
                            <span className="text-zinc-400 ml-auto">{new Date(r.completedAt).toLocaleDateString()}</span>
                          </div>
                        </button>
                      );
                    })}
        ...
        {engine === 'dojo' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto">
            ...
            <div className="bg-[#111113] border border-white/10 shadow-[0_1px_2px_rgba(0,0,0,0.3)] rounded-xl p-6">
              ...
              {badges.length === 0 ? (
                <p className="text-zinc-400 text-sm text-center py-8">No badges earned yet. Be the first.</p>
              ) : (
                <div className="space-y-3">
                  {badges.map(b => {
                    const emoji = b.tier === 'sensei' ? '👑' : b.tier === 'black' ? '🥋' : b.tier === 'blue' ? '🔵' : '⚪';
                    return (
                      <div key={b.badgeId} className="flex items-center gap-3 p-3 bg-[#1F1F23]/40 rounded-lg border border-white/10">
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto space-y-6">
      ...
      <div className={`rounded-xl border p-6 ${style.bg} ${style.border}`}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-xs font-mono text-zinc-400 mb-1">CROWD SIMULATION REPORT</p>
            <h2 className="text-xl font-bold">{report.productName}</h2>
            <p className="text-sm text-zinc-400">{report.totalPersonas} personas &middot; {new Date(report.completedAt).toLocaleString()}</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-[#111113] border border-red-500/10 shadow-[0_1px_2px_rgba(0,0,0,0.3)] rounded-xl p-6">
          ...
          <div className="space-y-2">
            ...
            {report.topFrictionPoints.length === 0 && (
              <p className="text-zinc-400 text-sm">No significant friction reported</p>
            )}
          </div>
        </div>

        <div className="bg-[#111113] border border-emerald-500/10 shadow-[0_1px_2px_rgba(0,0,0,0.3)] rounded-xl p-6">
          ...
          <div className="space-y-2">
            ...
            {report.topDelightPoints.length === 0 && (
              <p className="text-zinc-400 text-sm">No significant delight reported</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#111113] border border-white/10 shadow-[0_1px_2px_rgba(0,0,0,0.3)] rounded-xl p-6">
        ...
        <div className="space-y-3">
          {report.reactions.slice(0, 8).map((r, i) => {
            ...
            return (
              <div key={i} className="p-3 bg-[#1F1F23]/40 rounded-lg border border-white/10">
                <div className="flex items-center justify-between mb-1">
