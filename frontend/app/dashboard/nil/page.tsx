      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-sm font-normal text-slate-500">Name, Image &amp; Likeness</span>
          </div>
        ...
        >
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500">Available on {NIL_FOUNDATION.book.platform}</p>
          </div>
        </a>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {NIL_FOUNDATION.sections.map(section => {
          ...
          return (
            <div
              key={section.id}
              className={`bg-white shadow-sm border ${colors.border} rounded-xl p-5`}
            >
              ...
              <p className="text-sm text-slate-500 leading-relaxed whitespace-pre-line line-clamp-6">
      ...
      <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-5">
        ...
        <div className="space-y-3">
          {POSITION_NIL_PROFILES.map(profile => {
            ...
            return (
              <div key={profile.position} className="flex items-start gap-4 p-3 bg-slate-50 rounded-lg border border-slate-200">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-slate-500">{profile.sport}</span>
                  </div>
                  <p className="text-xs text-slate-500">{profile.valueDiver}</p>
                  <p className="text-xs text-slate-500 mt-1">{profile.challenge}</p>
                </div>
              </div>
            );
          })}
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] text-slate-500 uppercase tracking-wider">Value Range</span>
                <span className={`text-xs font-mono font-bold ${colors.text}`}>{deal.valueRange}</span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {deal.examples.map(ex => (
                  <span key={ex} className="px-2 py-0.5 bg-slate-100 rounded text-[10px] text-slate-500 border border-slate-200">
                    {ex}
                  </span>
                ))}
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {VALUATION_FACTORS.map(factor => {
          const colors = getColors(factor.color);
          return (
            <div key={factor.id} className={`bg-white shadow-sm border ${colors.border} rounded-xl p-5`}>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] text-slate-500 uppercase tracking-wider max-w-[200px] text-right">
                  {tier.prevalence}
                </span>
              </div>
              <p className="text-sm text-slate-500 mb-2">{tier.description}</p>
              <p className="text-xs text-slate-500 italic">{tier.archetype}</p>
            </div>
          );
        })}
      </div>
...
function PortalRevenueTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {portalSection && (
        <div className="bg-white shadow-sm border border-blue-500/30 rounded-xl p-6">
          ...
          <p className="text-sm text-slate-500 leading-relaxed whitespace-pre-line">{portalSection.content}</p>
        </div>
      )}

      {revenueSection && (
        <div className="bg-white shadow-sm border border-emerald-500/30 rounded-xl p-6">
          ...
          <p className="text-sm text-slate-500 leading-relaxed whitespace-pre-line">{revenueSection.content}</p>
        </div>
      )}

      {/* P.A.I. Formula — The Per|Form Grading Algorithm */}
      <div className="bg-white shadow-sm border border-amber-500/30 rounded-xl p-6">
        ...
        <div className="bg-slate-100 border border-slate-200 rounded-lg p-4 mb-4 text-center">
          ...
          <p className="text-xs text-slate-500 mt-2">Composite Score (0-100+) &middot; Proprietary weighting</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
          {PAI_FORMULA.components.map(comp => {
            ...
            return (
              <div key={comp.variable} className={`bg-slate-50 border ${colors.border} rounded-lg p-4`}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-mono">Weighted component</p>
                  </div>
                </div>
                <p className="text-xs text-slate-500 mb-2">{comp.description}</p>
                <div className="border-t border-slate-200 pt-2 mt-2">
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-0.5">Source Agent</p>
                  <p className="text-xs text-slate-600 font-medium">{comp.sourceAgent}</p>
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider mt-1.5 mb-0.5">Data Source</p>
                  <p className="text-xs text-slate-600">{comp.dataSource}</p>
                </div>
              </div>
            );
          })}
        ...
        {/* Tier Scale */}
        <h3 className="text-sm font-semibold text-slate-600 mb-3">Grade Scale</h3>
          ...
          {PAI_FORMULA.tiers.map(tier => {
            ...
            return (
              <div key={tier.label} className={`p-2 rounded-lg border ${colors.border} ${colors.bg} text-center`}>
                ...
                <p className="text-[10px] text-slate-500">{tier.scoreRange}</p>
                <p className="text-[9px] text-slate-500 mt-0.5">{tier.grade}</p>
              </div>
            );
          })}
        </div>
          ...
          {NIL_FOUNDATION.timeline.map((event, i) => (
            <div key={`${event.year}-${i}`} className="relative flex items-start gap-4 pl-1">
              ...
              <div className="relative z-10 mt-1.5">
                <div className="w-[14px] h-[14px] rounded-full bg-white border-2 border-amber-500/50 flex items-center justify-center">
                  <div className="w-[6px] h-[6px] rounded-full bg-amber-400" />
                </div>
              </div>

              {/* Content */}
              <div className="bg-white border border-slate-200 shadow-sm rounded-lg p-4 flex-1">
                <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="text-sm font-semibold text-slate-900 mb-1">{event.event}</h3>
                <p className="text-xs text-slate-500">{event.significance}</p>
              </div>
            </div>
          ))}
        </div>
