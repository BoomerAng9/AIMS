  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
        {[
          { label: "Registered", value: agents.length, color: "text-zinc-100" },
        ...
        {evolutionOpen && (
          <div className="mt-6 space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start">
              <div className="w-32 h-32 rounded-2xl border border-red-400/20 bg-[#1F1F23]/60 overflow-hidden shrink-0 flex items-center justify-center relative">
                <Image
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-zinc-500 leading-relaxed">
                  ...
                  Chicken Hawks do <strong className="text-zinc-400">not</strong> mentor — they assign work, enforce SOP,
            ...
            <div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {EVOLUTION_STAGES.map((stage) => (
                  <div key={stage.id} className="rounded-2xl border border-wireframe-stroke bg-[#1F1F23]/60 p-4">
                    <div className="h-24 rounded-xl border border-wireframe-stroke bg-[#18181B]/30 mb-3 flex items-center justify-center overflow-hidden relative">
                      <Image
                        src={stage.image}
                        alt={stage.visual}
                        fill
                        ...
                        <span
                          ...
                          className={`rounded-full border px-2 py-0.5 text-[9px] font-mono ${
                            hawk === squad.leaderRole
                              ...
                              : "border-amber-50/10 bg-gold/10 text-zinc-400"
                          }`}
                        >
                          {hawk}_LIL_HAWK
                        </span>
      ...
      <section className="rounded-3xl border border-wireframe-stroke bg-[#18181B]/70 p-6 backdrop-blur-2xl">
        ...
        {loading ? (
          <div className="flex items-center gap-2 py-8 justify-center">
            ...
            <span className="text-xs text-zinc-500">Loading registry...</span>
          </div>
        ) : agents.length === 0 ? (
          <div className="py-8 text-center">
            <p className="text-xs text-zinc-500">No agents registered or House of Ang unreachable</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {agents.map((ang) => (
                  <tr
                    key={ang.id}
                    className="border-b border-wireframe-stroke last:border-0 hover:bg-[#111113] transition-colors"
                  >
                    <td className="py-3 text-sm font-medium text-zinc-100 font-mono">{ang.name}</td>
                    <td className="py-3 text-xs text-zinc-400 max-w-xs truncate">{ang.description}</td>
                    <td className="py-3">
                      <div className="flex flex-wrap gap-1">
                        {ang.capabilities.slice(0, 3).map((cap) => (
                          <span key={cap} className="rounded-full border border-wireframe-stroke bg-[#18181B] px-2 py-0.5 text-[9px] text-gold font-mono">
                            {cap}
                          </span>
                        ))}
                        {ang.capabilities.length > 3 && (
                          <span className="text-[9px] text-zinc-600">+{ang.capabilities.length - 3}</span>
                        )}
                      </div>
                    </td>
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {agents.map((ang) => (
            <div
              key={ang.id}
              className="group relative overflow-hidden rounded-3xl border border-wireframe-stroke bg-[#18181B]/70 p-6 backdrop-blur-2xl transition-all hover:bg-white/80 hover:border-gold/20"
            >
      ...
      <section className="rounded-3xl border border-wireframe-stroke bg-[#18181B]/70 p-6 backdrop-blur-2xl">
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <div className="w-40 h-40 rounded-2xl border border-gold/20 bg-[#1F1F23]/60 overflow-hidden shrink-0 flex items-center justify-center relative">
            <Image
          ...
          <div className="flex-1 text-center md:text-left">
            <h3 className="text-sm font-semibold uppercase tracking-widest text-zinc-200 font-display mb-2">
              The Canon
            </h3>
      ...
      <section>
        ...
        <div
          className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-wireframe-stroke bg-[#18181B]/30 p-10 text-center transition-all hover:border-gold/20 cursor-pointer group"
          onClick={() => setSpawnOpen(!spawnOpen)}
        >
          <div className="flex h-14 w-14 items-center justify-center rounded-full border border-dashed border-white/10 text-zinc-600 group-hover:border-gold/30 group-hover:text-gold transition-all">
            <Brain size={24} />
          </div>
            ...
            <div className="mt-6 w-full max-w-lg space-y-4 text-left animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-wireframe-stroke bg-[#111113]/80 p-2.5 text-sm text-zinc-100 outline-none focus:border-gold/30 placeholder:text-zinc-600"
                />
              </div>
              <div className="space-y-2">
