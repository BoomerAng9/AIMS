  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total Plugs", value: plugs.length, color: "text-slate-800" },
      ...
      {!loading && !error && plugs.length === 0 && (
        <div className="wireframe-card border-dashed p-10 text-center">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl border border-dashed border-slate-200 text-slate-400">
            <Plus size={20} />
          </div>
          <p className="mt-4 text-sm text-slate-500">No plugs yet</p>
          <p className="mt-1 text-xs text-slate-400">
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-wireframe-stroke bg-white text-slate-400 group-hover:border-gold/30 group-hover:text-gold transition-colors">
                      <Layers size={18} />
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {plug.archetype && (
                          <span className="shrink-0 rounded border border-wireframe-stroke px-2 py-0.5 text-[0.55rem] font-mono text-slate-400 uppercase">
                            {plug.archetype}
                          </span>
                        )}
                      </div>
                      <p className="text-[0.6rem] text-slate-300 font-mono mt-0.5">
                        {plug.id}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                      ...
                      <div className="flex items-center gap-3">
                        {plug.domain && (
                          <a
                            ...
                            className="flex items-center gap-1 text-[0.6rem] text-slate-400 hover:text-gold transition-colors font-mono"
                          >
                            {plug.domain}
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                      <Link
                        ...
                        className="flex items-center gap-1 rounded-lg border border-wireframe-stroke px-2.5 py-1.5 text-[0.6rem] font-mono text-slate-500 hover:border-gold/20 hover:text-gold transition-all"
                      >
                        <Eye size={11} /> View
                      </Link>
      ...
      <div className="wireframe-card border-dashed p-8 text-center hover:border-gold/15 transition-colors group">
        <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-xl border border-dashed border-slate-200 text-slate-300 group-hover:border-gold/30 group-hover:text-gold transition-all">
          <Plus size={18} />
        </div>
        <p className="mt-3 text-sm text-slate-400 group-hover:text-slate-500 transition-colors">
          Build Another Plug
        </p>
        <Link
          href="/dashboard/build"
          className="mt-3 inline-flex items-center gap-2 rounded-lg border border-wireframe-stroke bg-slate-50 px-4 py-2 text-xs text-slate-500 hover:bg-gold hover:text-black hover:border-gold transition-all"
        >
          Open Build Wizard <ArrowRight size={12} />
        </Link>
      </div>
