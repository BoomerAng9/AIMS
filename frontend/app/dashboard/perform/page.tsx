    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-display text-white">Per|Form</h1>
        ...
        <div className="flex items-center gap-4 pt-2">
          ...
          {gridiron?.updatedAt && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-wireframe-stroke ml-auto">
      ...
      >
        {[
          ...
          <div key={stat.label} className="px-4 py-3 rounded-xl bg-white/[0.02] border border-wireframe-stroke">
        ...
        >
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-[11px] font-mono font-bold uppercase tracking-[0.3em] text-white/50">Live Gridiron Data</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            {/* ── Scoreboard ────────────────────────────────── */}
            {hasScoreboard && (
              <div className="rounded-xl border border-wireframe-stroke bg-black/30 overflow-hidden">
                <div className="px-4 py-3 border-b border-wireframe-stroke flex items-center justify-between">
                  <span className="text-xs font-mono font-bold uppercase tracking-widest text-white/50">Scoreboard</span>
                  <span className="text-[10px] font-mono text-white/25">{gridiron!.scoreboard.length} games</span>
                </div>
                  ...
                  {gridiron!.scoreboard.map((game, i) => (
                    <div key={i} className="px-4 py-2.5 flex items-center gap-3 hover:bg-white/[0.02] transition-colors">
                      <span className={`text-[9px] font-mono uppercase tracking-wider px-1.5 py-0.5 rounded ${
                        ...
                        game.status === 'FINAL' ? 'bg-white/5 text-white/30' :
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-medium text-white truncate">{game.awayTeam}</span>
                          <span className={`text-sm font-bold ${game.result === 'AWAY' ? 'text-gold' : 'text-white/60'}`}>
                            {game.awayScore ?? '—'}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-medium text-white truncate">{game.homeTeam}</span>
                          <span className={`text-sm font-bold ${game.result === 'HOME' ? 'text-gold' : 'text-white/60'}`}>
                            {game.homeScore ?? '—'}
                          </span>
                        </div>
                      </div>
                      <span className="text-[9px] font-mono text-white/20 flex-shrink-0">{game.sport}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {hasStandings && (
              <div className="rounded-xl border border-wireframe-stroke bg-black/30 overflow-hidden">
                <div className="px-4 py-3 border-b border-wireframe-stroke flex items-center justify-between">
                  <span className="text-xs font-mono font-bold uppercase tracking-widest text-white/50">Standings</span>
                  <span className="text-[10px] font-mono text-white/25">Top 25</span>
                </div>
                <div className="divide-y divide-wireframe-stroke max-h-[300px] overflow-y-auto">
                  {gridiron!.standings.map((team, i) => (
                    <div key={i} className="px-4 py-2 flex items-center gap-3 hover:bg-white/[0.02] transition-colors">
                      <span className="text-[10px] font-bold text-white/20 w-5 text-right">
                        {team.apRank || team.cfpRank || i + 1}
                      </span>
                      <div className="flex-1 min-w-0">
                        <span className="text-xs font-medium text-white truncate block">{team.team}</span>
                        <span className="text-[10px] font-mono text-white/25">{team.conference}</span>
                      </div>
                      <span className="text-xs font-mono text-white/50">{team.wins}-{team.losses}</span>
                      <span className="text-[10px] font-mono text-white/25">({team.confWins}-{team.confLosses})</span>
                      {team.bowlGame && (
            ...
            {hasPortal && (
              <div className="rounded-xl border border-wireframe-stroke bg-black/30 overflow-hidden">
                <div className="px-4 py-3 border-b border-wireframe-stroke flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-mono font-bold uppercase tracking-widest text-white/50">Portal Feed</span>
                ...
                <div className="divide-y divide-wireframe-stroke max-h-[300px] overflow-y-auto">
                  {gridiron!.portalMoves.map((move, i) => (
                    <div key={i} className="px-4 py-2.5 hover:bg-white/[0.02] transition-colors">
                      <div className="flex items-center gap-2">
                        <Circle className={`w-2 h-2 flex-shrink-0 ${PORTAL_STATUS_COLORS[move.status] || 'text-white/20'}`} fill="currentColor" />
                        <span className="text-xs font-medium text-white truncate">{move.playerName}</span>
                        <span className="text-[10px] font-mono text-white/25 flex-shrink-0">{move.position}</span>
                        {move.paiScore && (
            ...
            {hasCoaching && (
              <div className="rounded-xl border border-wireframe-stroke bg-black/30 overflow-hidden">
                <div className="px-4 py-3 border-b border-wireframe-stroke flex items-center justify-between">
                  <span className="text-xs font-mono font-bold uppercase tracking-widest text-white/50">Coaching Carousel</span>
                    ...
                    <div key={i} className="px-4 py-2.5 hover:bg-white/[0.02] transition-colors">
                      <div className="flex items-center gap-2">
                        <span className={`text-[9px] font-mono font-bold uppercase px-1.5 py-0.5 rounded border ${
                          CHANGE_TYPE_COLORS[change.changeType] || 'text-white/40 bg-white/5 border-white/10'
                        }`}>
                          {change.changeType}
                        </span>
                        <span className="text-xs font-medium text-white truncate">{change.coachName}</span>
                      </div>
                      <div className="flex items-center gap-2 mt-1 text-[10px] font-mono text-white/30">
                        ...
                        {change.record && <span className="text-white/20">({change.record})</span>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {hasDraft && (
              <div className="rounded-xl border border-wireframe-stroke bg-black/30 overflow-hidden">
                <div className="px-4 py-3 border-b border-wireframe-stroke flex items-center justify-between">
                  <span className="text-xs font-mono font-bold uppercase tracking-widest text-white/50">Draft Board</span>
                ...
                <div className="divide-y divide-wireframe-stroke max-h-[300px] overflow-y-auto">
                  {gridiron!.draftBoard.map((prospect, i) => (
                    <div key={i} className="px-4 py-2.5 flex items-center gap-3 hover:bg-white/[0.02] transition-colors">
                      <span className="text-lg font-bold text-white/10 w-6 text-right flex-shrink-0">{prospect.overallRank}</span>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-white truncate">{prospect.name}</p>
                        <p className="text-[10px] font-mono text-white/25">{prospect.college}</p>
                      </div>
                      <span className="text-[10px] text-white/20">{TREND_ARROW[prospect.trend] || '—'}</span>
            ...
            {hasNil && (
              <div className="rounded-xl border border-wireframe-stroke bg-black/30 overflow-hidden">
                <div className="px-4 py-3 border-b border-wireframe-stroke flex items-center justify-between">
                  <span className="text-xs font-mono font-bold uppercase tracking-widest text-white/50">NIL Leaderboard</span>
                ...
                <div className="divide-y divide-wireframe-stroke max-h-[300px] overflow-y-auto">
                  {gridiron!.nilLeaders.map((entry, i) => (
                    <div key={i} className="px-4 py-2.5 flex items-center gap-3 hover:bg-white/[0.02] transition-colors">
                      <span className="text-[10px] font-bold text-white/20 w-5 text-right">#{entry.rank}</span>
                      <div className="flex-1 min-w-0">
                        <span className="text-xs font-medium text-white truncate block">{entry.team}</span>
                        <span className="text-[10px] font-mono text-white/20">{entry.dealCount} deals</span>
                      </div>
                      <span className="text-[10px] text-white/20">{TREND_ARROW[entry.trend] || '—'}</span>
                      <div className="text-right flex-shrink-0">
                        <p className="text-xs font-bold text-green-400">{formatCurrency(entry.totalNilValue)}</p>
                        <p className="text-[9px] font-mono text-white/20">{formatCurrency(entry.avgPerPlayer)}/player</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {gridiron && gridiron.portalSummary.total > 0 && (
            <div className="flex items-center gap-4 px-4 py-3 rounded-xl bg-white/[0.02] border border-wireframe-stroke">
              <ArrowRightLeft className="w-4 h-4 text-purple-400/50 flex-shrink-0" />
              <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-white/30">Portal Summary</span>
              <div className="flex items-center gap-4 ml-auto text-[11px] font-mono">
                <span className="text-amber-400">{gridiron.portalSummary.inPortal} <span className="text-white/20">active</span></span>
                <span className="text-emerald-400">{gridiron.portalSummary.committed} <span className="text-white/20">committed</span></span>
                <span className="text-gold">{gridiron.portalSummary.signed} <span className="text-white/20">signed</span></span>
                <span className="text-white/30">{gridiron.portalSummary.total} total</span>
              </div>
            </div>
          )}
        </motion.div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          <span className="text-[11px] font-mono font-bold uppercase tracking-[0.3em] text-white/30">Platform</span>
          <div className="flex-1 h-px bg-wireframe-stroke" />
        </div>
              ...
              <motion.div key={card.title} variants={staggerItem}>
                <Link href={card.href}>
                  <div className="group rounded-xl border border-wireframe-stroke bg-black/40 hover:bg-black/60 hover:border-gold/20 transition-all p-5 h-full flex flex-col gap-3 cursor-pointer">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-white group-hover:text-gold transition-colors">{card.title}</h3>
                      <p className="text-xs text-white/30 mt-1 leading-relaxed">{card.description}</p>
                    </div>
                  </div>
                </Link>
              </motion.div>
