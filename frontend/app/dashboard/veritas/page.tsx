...
const RISK_STYLES = {
  ...
  unverified: { bg: 'bg-[#1F1F23]', text: 'text-zinc-400', border: 'border-white/15', label: 'UNVERIFIED' },
      ...
      <div className="min-h-screen bg-[#F8FAFC] text-zinc-100 p-4 md:p-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-2 text-xs rounded-lg border transition-colors ${
                view === 'upload' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'text-zinc-400 border-white/10 hover:border-white/15'
              }`}
            >
              New Analysis
            </button>
            <button
              ...
              className={`px-3 py-2 text-xs rounded-lg border transition-colors ${
                view === 'history' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'text-zinc-400 border-white/10 hover:border-white/15'
              }`}
            >
        ...
        {view === 'upload' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-[#1F1F23]/60 border border-white/10 rounded-lg text-sm text-zinc-100 outline-none focus:border-red-500/30"
                />
              </div>
              <div>
                <label className="text-xs text-zinc-400 mb-1 block">Document Name</label>
                <input
                  value={documentName}
            ...
            <div>
              <label className="text-xs text-zinc-400 mb-1 block">Document Content</label>
              <textarea
                value={documentText}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto space-y-4">
            ...
            <div className="bg-[#111113] border border-white/10 shadow-[0_1px_2px_rgba(0,0,0,0.3)] rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-xs font-mono text-zinc-400">AGENT ACTIVITY — WATCH MODE</p>
              </div>
              <div ref={eventLogRef} className="h-64 overflow-y-auto space-y-1 font-mono text-xs">
                ...
                {events.length === 0 && (
                  <p className="text-zinc-400">Waiting for pipeline to start...</p>
                )}
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto">
            {history.length === 0 ? (
              <div className="text-center py-20">
                ...
                <p className="text-zinc-400 text-sm mt-1">Submit a document to generate your first verification report</p>
              </div>
            ) : (
                ...
                {history.map(r => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-zinc-400">{r.documentName}</p>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto space-y-6">
      ...
      <div className={`rounded-xl border p-6 ${riskStyle.bg} ${riskStyle.border}`}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-xs font-mono text-zinc-400 mb-1">VERITAS REPORT</p>
            <h2 className="text-xl font-bold">{report.projectName}</h2>
            <p className="text-sm text-zinc-400">{report.documentName} &middot; {new Date(report.completedAt).toLocaleString()}</p>
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {finding.variancePercent !== null && (
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xs text-zinc-400">Variance:</span>
...
function MiniStat({ label, value, color }: { label: string; value: number; color?: string }) {
  const textColor = color === 'red' ? 'text-red-400' : color === 'amber' ? 'text-amber-400' : color === 'emerald' ? 'text-emerald-400' : 'text-zinc-100';
  return (
    <div className="text-center">
