...
function ChartTooltip({ active, payload, label, unit = "" }: { active?: boolean; payload?: Array<{ value: number; name: string; color: string }>; label?: string; unit?: string }) {
  ...
  return (
    <div className="rounded-xl border border-wireframe-stroke bg-black/95 backdrop-blur-xl px-3 py-2 shadow-xl">
      ...
      {services.map((svc) => {
        ...
        return (
          <motion.div
            ...
            className={`rounded-xl border ${borderColor} bg-black/40 p-3 cursor-default transition-all hover:bg-white/[0.02]`}
          >
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-1">
              ...
              <div>
                <p className="text-[9px] text-white/30">Uptime</p>
                <p className={`text-xs font-mono font-semibold ${statusColor}`}>{svc.uptime}%</p>
              </div>
              <div>
                <p className="text-[9px] text-white/30">Req/hr</p>
                <p className="text-xs font-mono text-white/60">{svc.requests.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-[9px] text-white/30">Errors</p>
                <p className={`text-xs font-mono ${svc.errors > 5 ? "text-red-400" : "text-white/40"}`}>{svc.errors}</p>
              </div>
            </div>
            <p className="text-[8px] text-white/20 mt-2 text-right">{svc.lastCheck}</p>
          </motion.div>
        );
      })}
    </div>
...
export default function OperationsPage() {
  ...
  return (
    ...
    >
      {/* ── Header Strip ── */}
      <motion.section variants={staggerItem} className="rounded-2xl border border-gold/20 bg-gradient-to-r from-gold/10 via-black/80 to-black/60 p-4 shadow-[0_0_40px_rgba(251,191,36,0.06)]">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white font-display">Operations Command</h1>
              <p className="text-[10px] text-white/30 uppercase tracking-widest">Platform Telemetry &amp; Observability</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {/* Live indicator */}
            <button onClick={() => setIsLive(!isLive)} className="flex items-center gap-2 rounded-lg border border-wireframe-stroke bg-black/40 px-3 py-1.5 transition-all hover:border-gold/20">
              ...
              <span className={`text-[10px] font-bold uppercase tracking-widest ${isLive ? "text-emerald-400" : "text-white/30"}`}>
                {isLive ? "LIVE" : "PAUSED"}
              </span>
            </button>

            {/* Time range selector */}
            <div className="flex items-center rounded-lg border border-wireframe-stroke bg-black/40 p-0.5">
              {(["1h", "6h", "24h", "7d", "30d"] as TimeRange[]).map((range) => (
                <button
                  key={range}
                  onClick={() => setTimeRange(range)}
                  className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${
                    timeRange === range ? "bg-gold/10 text-gold border border-gold/20" : "text-white/30 hover:text-white/50"
                  }`}
                >
                  {range}
                </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[
          ...
          <motion.div
            ...
            className="rounded-xl border border-wireframe-stroke bg-black/60 p-4 backdrop-blur-xl hover:border-white/10 transition-all"
          >
            <div className="flex items-center justify-between mb-1">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-white/40">{kpi.label}</p>
            ...
            <div className="flex items-center gap-1 mt-0.5">
              ...
              <span className="text-[10px] text-white/30">{kpi.trendLabel}</span>
            </div>
          ...
          >
            ...
            {tab.count !== undefined && (
              <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold ${
                activeTab === tab.key ? "bg-gold/10 text-gold" : "bg-white/5 text-white/20"
              }`}>
                {tab.count}
              </span>
            )}
          ...
          {activeTab === "overview" && (
            <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-wireframe-stroke bg-black/60 p-5 backdrop-blur-xl">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-white">Response Time (p50)</h3>
                    </div>
                    <span className="text-xs text-white/30 font-mono">{timeRange}</span>
                  </div>
                ...
                <div className="rounded-2xl border border-wireframe-stroke bg-black/60 p-5 backdrop-blur-xl">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-white">Request Rate (by Status)</h3>
                    </div>
                    <div className="flex items-center gap-3">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {/* Service Health Grid */}
                <div className="lg:col-span-2 rounded-2xl border border-wireframe-stroke bg-black/60 p-5 backdrop-blur-xl">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-white/30">{healthyCount}/{services.length} services healthy</span>
                      <RefreshCw size={12} className="text-white/20 animate-spin" style={{ animationDuration: "3s" }} />
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {/* Live Event Feed */}
                <div className="lg:col-span-2 rounded-2xl border border-wireframe-stroke bg-black/60 backdrop-blur-xl overflow-hidden">
                    ...
                    {liveEvents.map((evt, i) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="flex items-start gap-3 px-4 py-2.5 border-b border-wireframe-stroke/30 hover:bg-white/[0.02] transition-colors"
                        >
                          <span className={`h-2 w-2 rounded-full mt-1.5 flex-shrink-0 ${dotColors[evt.severity]}`} />
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] text-white/20 font-mono">{evt.time}</span>
          ...
          {activeTab === "alerts" && (
            <div className="rounded-2xl border border-wireframe-stroke bg-black/60 p-5 backdrop-blur-xl">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold uppercase tracking-widest text-white/80">Active Alerts</h2>
              </div>
                    ...
                    <thead>
                      <tr className="border-b border-wireframe-stroke">
                        {["ID", "Severity", "Metric", "Threshold", "Current", "Fired"].map(h => (
                          <th key={h} className="p-3 text-left text-[10px] uppercase tracking-widest text-white/30 font-semibold">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {ACTIVE_ALERTS.map((alert) => (
                        <tr key={alert.id} className="border-t border-wireframe-stroke hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="p-3"><code className="rounded bg-black/60 border border-wireframe-stroke px-2 py-0.5 text-[10px] font-mono text-white/40">{alert.threshold}</code></td>
                          <td className="p-3">
          ...
          {activeTab === "incidents" && (
            <div className="rounded-2xl border border-wireframe-stroke bg-black/60 p-5 backdrop-blur-xl">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold uppercase tracking-widest text-white/80">Incident Timeline</h2>
              </div>
              {/* Timeline view */}
                ...
                {INCIDENTS.map((inc) => {
                  ...
                  return (
                    <div key={inc.id} className="relative">
                      ...
                      <div className={`rounded-xl border ${isOpen ? "border-red-500/20 bg-red-500/5" : "border-wireframe-stroke bg-black/40"} p-4`}>
                        <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
                          ...
                          <div className="flex items-center gap-3 text-[10px] text-white/30">
                            {inc.duration && <span>Duration: <span className="text-white/50 font-mono">{inc.duration}</span></span>}
          ...
          {activeTab === "traces" && (
            <div className="rounded-2xl border border-wireframe-stroke bg-black/60 p-5 backdrop-blur-xl">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold uppercase tracking-widest text-white/80">Request Traces</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-wireframe-stroke">
                      {["Correlation ID", "Time", "Method", "Path", "Status", "Duration"].map(h => (
                        <th key={h} className={`p-3 text-[10px] uppercase tracking-widest text-white/30 font-semibold ${h === "Status" || h === "Duration" ? "text-center" : "text-left"}`}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {CORRELATION_TRACES.map((trace) => {
                      ...
                      return (
                        <tr key={trace.correlationId} className="border-t border-wireframe-stroke hover:bg-white/[0.02] transition-colors">
                          <td className="p-3">
                            <code className="rounded bg-black/60 border border-wireframe-stroke px-2 py-0.5 text-[10px] font-mono text-gold">{trace.correlationId}</code>
                          </td>
                          <td className="p-3">
                            <span className="text-[10px] font-mono text-white/30">{trace.timestamp}</span>
                          </td>
                          <td className="p-3">
                          ...
                          <td className="p-3"><code className="text-xs font-mono text-white/50">{trace.path}</code></td>
