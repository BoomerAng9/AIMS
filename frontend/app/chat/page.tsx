    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-50 w-52 max-h-64 overflow-y-auto rounded-xl border border-white/10 bg-[#18181B]/95 backdrop-blur-xl shadow-2xl">
            {(['elevenlabs', 'deepgram'] as const).map(prov => (
              <div key={prov}>
                <div className="px-3 py-1.5 border-b border-white/8">
                  <span className="text-[9px] text-zinc-500 font-mono uppercase tracking-wider">{prov}</span>
                </div>
                {TTS_VOICES.filter(v => v.provider === prov).map(v => (
                  ...
                  >
                    <span>{v.name}</span>
                    <span className="text-zinc-600">{v.style}</span>
                  </button>
                ))}
              </div>
            ))}
    ...
    >
      ...
      <div className={`min-w-0 max-w-[80%] ${isUser ? 'ml-auto' : ''}`}>
        <div className={`rounded-2xl px-4 py-2.5 text-sm leading-relaxed break-words overflow-hidden ${
          isUser
            ? 'bg-gold/10 text-zinc-100 rounded-tr-sm border border-gold/15'
            : 'bg-[#18181B] backdrop-blur-sm text-zinc-300 rounded-tl-sm border border-white/10 shadow-[0_2px_8px_rgba(0,0,0,0.4)]'
        }`}>
          {isUser ? (
            <p className="whitespace-pre-wrap break-words">{content}</p>
          ) : (
            <div className="prose prose-invert prose-sm max-w-none break-words
              prose-headings:text-zinc-100 prose-a:text-gold prose-strong:text-zinc-200
...
function ThreadsSidebar({ threads, activeId, onSelect, onNew, onDelete, open, onToggle }: {
  ...
  return (
    <motion.aside
      ...
      className="flex-shrink-0 overflow-hidden border-r border-white/8 bg-[#111113]"
    >
      <div className="flex flex-col h-full w-[240px]">
        <div className="flex items-center justify-between px-3 py-2.5 border-b border-white/8">
          <span className="text-[9px] uppercase tracking-[0.15em] text-zinc-500 font-mono">Threads</span>
          <div className="flex gap-1">
            <button onClick={onNew} className="p-1 rounded text-zinc-500 hover:text-gold transition-colors"><Plus size={13} /></button>
            <button onClick={onToggle} className="p-1 rounded text-zinc-500 hover:text-zinc-300 transition-colors"><PanelLeftClose size={13} /></button>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto py-1.5 space-y-0.5">
          {threads.length === 0 ? (
            <div className="px-3 py-8 text-center">
              <MessageSquare className="w-5 h-5 text-zinc-600 mx-auto mb-1.5" />
              <p className="text-[9px] text-zinc-600 font-mono">No threads yet</p>
            </div>
          ) : threads.map(t => (
            <div
              key={t.id}
      ...
      <div className="rounded-2xl border border-gold/15 bg-gradient-to-b from-gold/[0.04] to-transparent backdrop-blur-sm p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <div className="rounded-lg bg-[#111113] border border-white/10 p-1">
              <FrequencyVisualizer
...
export default function ChatPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-dvh bg-[#09090B]"><Loader2 className="w-8 h-8 animate-spin text-gold" /></div>}>
      <ChatContent />
    </Suspense>
  );
}
      ...
      <div className="h-dvh w-full flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex overflow-hidden min-h-0">
          ...
          <main className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
            {/* ── Header ── */}
            <div className="flex-shrink-0 flex items-center gap-3 px-4 py-2.5 border-b border-white/8 bg-[#09090B]/90 backdrop-blur-xl">
              {/* Left */}
              <div className="flex items-center gap-2.5 min-w-0">
                {!sidebarOpen && (
                  <button onClick={toggleSidebar} className="p-1 rounded text-zinc-500 hover:text-gold transition-colors flex-shrink-0">
                    <PanelLeftOpen size={15} />
                  </button>
                )}
                <div className="relative flex-shrink-0">
                  <div className="w-8 h-8 rounded-lg bg-[#111113] border border-gold/15 overflow-hidden">
                    <Image src="/images/acheevy/acheevy-helmet.png" alt="ACHEEVY" width={32} height={32} className="w-full h-full object-cover" />
                  </div>
                  <div className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border-[1.5px] border-[#09090B] ${
                    voiceActive ? 'bg-gold animate-pulse' : 'bg-emerald-400'
                  }`} />
                </div>
                <div className="min-w-0">
                  <h1 className="font-semibold text-sm text-zinc-100 tracking-tight truncate">Chat w/ACHEEVY</h1>
                  <p className="text-[9px] text-zinc-500 font-mono truncate">
                    {voiceActive ? 'Voice Session Active' : 'AI Orchestrator'}
                  </p>
                </div>
              </div>
              ...
              <div className="max-w-2xl mx-auto px-4 py-4 space-y-4 w-full">
                ...
                {messages.length === 0 && !voiceActive && (
                  ...
                  >
                    ...
                    <h2 className="text-lg font-semibold text-zinc-100 mb-1">Chat w/ACHEEVY</h2>
                ...
                {isLoading && messages.length > 0 && messages[messages.length - 1].role === 'user' && (
                  <div className="flex gap-3">
                    ...
                    <div className="px-3 py-2.5 bg-[#18181B] rounded-2xl rounded-tl-sm border border-white/10 flex items-center gap-1.5">
            ...
            {/* ── Input Area ── */}
            <div className="flex-shrink-0 border-t border-white/8 bg-[#09090B]/90 backdrop-blur-xl px-4 py-3">
              <div className="max-w-2xl mx-auto w-full">
                ...
                <form id="chat-form" onSubmit={handleEnhancedSubmit}>
                  <div className="flex items-end gap-2 rounded-2xl border border-white/10 bg-[#111113] p-2.5 focus-within:border-gold/20 transition-colors">
                    {/* File attach */}
                    <button
                      type="button"
                      onClick={() => fileRef.current?.click()}
                      className="p-2 rounded-xl bg-transparent text-zinc-500 hover:text-gold/60 hover:bg-gold/[0.06] transition-all flex-shrink-0"
                    >
                    ...
                    <textarea
                      ...
                      rows={1}
                      className="flex-1 bg-transparent text-zinc-100 placeholder:text-zinc-600 resize-none outline-none text-sm leading-relaxed max-h-[140px] py-1.5 min-w-0"
                    />
                    ...
                    <button
                      ...
                      className={`p-2 rounded-xl transition-all flex-shrink-0 ${
                        isSpeaking ? 'bg-gold/15 text-gold animate-pulse'
                          : ttsEnabled ? 'bg-gold/[0.06] text-gold/60' : 'bg-transparent text-zinc-600'
                      }`}
