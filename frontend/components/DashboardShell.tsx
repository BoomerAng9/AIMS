  ...
  return (
    <LogoWallBackground mode="dashboard">
      ...
      <div className={`flex min-h-full ${IS_DEMO ? "pt-9" : "pt-[env(safe-area-inset-top)]"}`}>
        ...
        <aside className="hidden w-64 flex-shrink-0 flex-col border-r border-white/8 bg-[#111113] lg:flex z-30 sticky top-0 h-screen">
          <div className="px-4 py-5">
            <div className="flex flex-col">
              <span className="font-display text-sm uppercase tracking-wider text-zinc-100">
                A.I.M.S.
              </span>
              <span className="text-[0.5rem] uppercase tracking-[0.12em] text-zinc-500 -mt-0.5">
          ...
          {/* Live status card */}
          <div className="mx-3 mb-3 rounded-xl border border-white/8 bg-[#18181B] px-3 py-3 text-[0.75rem] text-zinc-400">
            <div className="flex items-center gap-2">
              <span
                className={`h-2 w-2 rounded-full ${statusDotClass(health.status)}`}
              />
              <p className="text-[0.6rem] uppercase tracking-[0.18em] text-amber-500/70 font-mono">
        ...
        <div className="flex flex-1 flex-col min-h-full relative z-10">
          {/* Top bar */}
          <header className="sticky top-0 flex items-center justify-between border-b border-white/8 bg-[#09090B]/80 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8 xl:px-12 z-20 shadow-none">
            <div className="flex items-center gap-5">
              {/* Navigation Controls */}
              <div className="flex items-center gap-2 -ml-2">
                <button
                  onClick={() => router.back()}
                  className="p-2.5 text-zinc-500 hover:text-amber-500 hover:bg-amber-500/10 rounded-xl transition-all duration-200 border border-transparent hover:border-amber-500/30"
                  title="Go Back"
                >
              ...
              <div className="flex flex-col">
                <span className="text-[0.65rem] uppercase tracking-[0.18em] text-amber-500/70 font-mono">
                  Dashboard
                </span>
            ...
            <div className="flex items-center gap-3">
              {/* LUC pill */}
              <div className="hidden items-center gap-2 rounded-lg border border-white/10 bg-[#111113] px-3 py-1.5 text-xs text-zinc-400 sm:flex">
                <span
                  className={`h-2 w-2 rounded-full ${statusDotClass(health.status)}`}
                />
                <span className="font-mono text-[0.65rem]">LUC</span>
                <span className="text-amber-500 font-semibold">{balance}</span>
              </div>
                    ...
                    <div className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-white/10 bg-[#18181B] shadow-lg shadow-black/50 z-40 overflow-hidden">
                      ...
                      <div className="py-1">
                        <button
                          ...
                          className="w-full text-left px-4 py-2 text-sm text-zinc-400 hover:bg-white/5 hover:text-zinc-100 transition-colors"
                        >
                          Settings
                        </button>
                        <button
