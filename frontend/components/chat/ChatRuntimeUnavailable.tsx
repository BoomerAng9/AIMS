...
export interface ChatRuntimeUnavailableProps {
  entryLabel: string;
}

export function ChatRuntimeUnavailable({ entryLabel }: ChatRuntimeUnavailableProps) {
  return (
    <div className="flex min-h-[calc(100vh-64px)] items-center justify-center bg-[#0b0b0c] px-6 py-10 text-zinc-100">
      <div className="w-full max-w-2xl rounded-3xl border border-amber-500/20 bg-[#121214] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.45)]">
        <div className="mb-5 flex items-center gap-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-amber-400/20 bg-amber-500/10 shadow-[0_0_32px_rgba(245,158,11,0.12)]">
            <Image
          ...
          <div className="inline-flex rounded-full border border-amber-400/20 bg-amber-500/10 px-3 py-1 text-[11px] uppercase tracking-[0.28em] text-amber-300">
            CHAT W/ ACHEEVY
          </div>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-zinc-300">
          ...
          <p className="mt-2 font-mono text-xs text-amber-200">CHAT_RUNTIME_URL=https://chat.your-domain.com</p>
          <p className="mt-1 font-mono text-xs text-amber-200">CHAT_RUNTIME_BRIDGE_SECRET=generate-a-random-secret</p>
