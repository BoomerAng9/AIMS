...
function AuthModal({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-50/80 backdrop-blur-sm"
    ...
    >
      <motion.div
        ...
        className="relative w-full max-w-sm mx-4 rounded-2xl border border-gold/20 bg-white/95 backdrop-blur-xl p-8 shadow-2xl shadow-gold/5"
      >
        {/* Close button */}
        <button
          onClick={onClose}
        ...
        <div className="space-y-3 mb-4">
          <button
            ...
            className="group relative w-full h-11 overflow-hidden rounded-lg bg-slate-50 border border-slate-200 hover:border-gold/30 hover:bg-slate-100 transition-all disabled:opacity-50 flex items-center justify-center gap-3"
          >
            {isLoading === 'google' ? (
          ...
          <button
            ...
            disabled={isLoading !== null}
            className="group relative w-full h-11 overflow-hidden rounded-lg bg-slate-50 border border-slate-200 hover:border-[#5865F2]/40 hover:bg-[#5865F2]/10 transition-all disabled:opacity-50 flex items-center justify-center gap-3"
          >
            {isLoading === 'discord' ? (
              ...
              <>
                ...
                <span className="text-sm text-slate-700">Continue with Discord</span>
              </>
            )}
          </button>
        </div>
        ...
        <div className="text-center space-y-2">
          <div className="flex items-center gap-3 text-[0.65rem] uppercase tracking-widest text-slate-300">
          ...
          <div className="flex justify-center gap-4 text-sm">
            ...
            <Link href="/sign-up" className="text-slate-500 hover:text-slate-800 transition-colors">
              Create Account
            </Link>
          </div>
        </div>
