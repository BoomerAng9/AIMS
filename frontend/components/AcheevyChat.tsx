...
export default function AcheevyChat() {
  ...
  useEffect(() => { voiceOutputRef.current = voiceOutput; });
  ...
  const handlePause = useCallback(() => { voiceOutputRef.current.pause(); }, []);
  const handleReplay = useCallback((id: string, content: string) => { handleSpeak(id, content); }, [handleSpeak]);
    ...
    <div className="flex flex-col h-[calc(100dvh-3.5rem)] relative overflow-hidden bg-[#0A0A0A] text-white font-sans">
      ...
      <div
        ...
        style={{
          backgroundImage: `
            ...
            url(/images/acheevy/acheevy-helmet-chat.png)
          `,
      ...
      <div className="relative z-20 p-3 bg-[#0A0A0A]/80 backdrop-blur-xl border-t border-wireframe-stroke">
        ...
        >
          ...
          {isLoading ? (
            <button type="button" onClick={stop} className="absolute right-2 top-1.5 p-2 bg-red-500/10 text-red-400 rounded-lg transition-all hover:bg-red-500/20 border border-transparent hover:border-red-500/30">
              <Square className="w-4 h-4" />
            </button>
          ) : (
            <button type="submit" disabled={!input.trim() && attachments.length === 0} className="absolute right-2 top-1.5 p-2 bg-gold/10 hover:bg-gold text-gold hover:text-black rounded-lg transition-all disabled:opacity-30 border border-gold/30 hover:border-gold">
