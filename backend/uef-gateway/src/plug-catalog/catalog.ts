...
const PLUG_REGISTRY: PlugDefinition[] = [
  ...
  // ── Seedance 2.0 — Video Generation ──────────────────────────────────
  {
    ...
    description: 'Autonomous video generation pipeline. Feed it a product link and it produces platform-ready UGC content. Handles asset extraction, creative brief generation, video synthesis, and multi-platform formatting. Integrates with Chicken Hawk for end-to-end automation.',
    category: 'content-engine',
    tags: ['video', 'ugc', 'content', 'marketing', 'automation', 'ai-video', 'tiktok', 'ads'],
    tier: 'pro',
    version: '2.0.0',
    sourceUrl: 'https://seedance-2ai.org',
    ...
    healthCheck: { endpoint: '/health', interval: '30s', timeout: '10s', retries: 3, startPeriod: '30s' },
    envVars: [
      { key: 'SEEDANCE_API_KEY', description: 'Seedance 2.0 API key', required: true, sensitive: true, category: 'api-key' },
  ...
  // ── Kling — Advanced Video AI ────────────────────────────────────────
  {
    ...
    description: 'Production-grade video generation with advanced motion control, lip sync, and scene composition. Integrates with the content pipeline for high-quality marketing material.',
    category: 'content-engine',
    tags: ['video', 'ai-video', 'motion', 'lip-sync', 'production', 'ads'],
    tier: 'pro',
    version: '2.0.0',
    license: 'Commercial API',
    ...
    healthCheck: { endpoint: '/health', interval: '30s', timeout: '10s', retries: 3, startPeriod: '20s' },
    envVars: [
      { key: 'KLING_ACCESS_KEY', description: 'Kling API access key', required: true, sensitive: true, category: 'api-key' },
      { key: 'KLING_SECRET_KEY', description: 'Kling API secret key', required: true, sensitive: true, category: 'api-key' },
    ],
    networkPolicy: { internetAccess: true, allowedEgress: ['api.klingai.com'], isolatedSandbox: false, bridgeToAims: true },
    securityLevel: 'standard',
    customizations: [
      { id: 'model', label: 'Model Version', description: 'Kling model to use', type: 'select', options: ['kling-v2-master', 'kling-v2-standard'], default: 'kling-v2-master' },
