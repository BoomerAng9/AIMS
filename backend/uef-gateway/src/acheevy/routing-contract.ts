...
export function resolveIntentFromRoutingDecision(routingDecision: NtntnRoutingDecision): string {
  if (routingDecision.intent) {
    return normalizeCompatibilityIntent(routingDecision.intent);
  }

  if (routingDecision.intent_type === 'vertical' && routingDecision.vertical_id) {
    return `vertical:${routingDecision.vertical_id}`;
  }

  switch (routingDecision.intent_type) {
    case 'plug_fabrication':
      return 'plug-factory:custom';
    case 'research':
...
export function normalizeCompatibilityIntent(intent: string): string {
  if (intent.startsWith('paas-')) {
...
interface LegacyAcheevyClassification {
  intent: string;
  confidence: number;
  requiresAgent: boolean;
}
