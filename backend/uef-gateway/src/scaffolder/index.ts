        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <section className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Summary metrics and key information for ${title.toLowerCase()}.
            </p>
          </section>
...
function generateApiRoute(route: string): ScaffoldedFile {
  ...
  try {
    ...
    if (!${paramName}) {
      return Response.json({ error: '${paramName} is required' }, { status: 400 });
    }
    // Replace with actual data fetch (e.g., from database or service)
    const record = { ${paramName}, message: '${routeName} retrieved' };
  ...
  try {
    ...
    // Replace with actual data fetch (e.g., from database or service)
    const data: unknown[] = [];
    return Response.json({ data, page, limit, total: 0 });
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Internal server error';
    return Response.json({ error: message }, { status: 500 });
  }
...
export async function POST(req: Request) {
  try {
    ...
    // Replace with actual creation logic (e.g., insert into database)
 ...
 *
 * Uses an in-memory store by default. Replace the store
 * with your database client (Prisma, Drizzle, Knex, etc.)
 * while keeping the same interface.
 */
const store = new Map<string, ${pascal}>();

export const ${camel}Repository = {
  async findAll(filter?: { status?: string }): Promise<${pascal}[]> {
    const all = Array.from(store.values());
    if (filter?.status) {
      return all.filter(item => item.status === filter.status);
    }
    return all;
  },
...
`,
    type: 'test',
    description: 'Rendering tests for the home page',
  };
}
