...
## 2024-05-25 - Promise.all Refactoring for Disk I/O
**Learning:** Sequential disk read operations using `readFile` inside `for...of` loops cause synchronous bottlenecking, even when awaited, reducing startup efficiency. Converting `for...of` file loops to `Promise.all` with `map` dramatically speeds up operations like system initialization.
**Action:** Replace `for...of` loops performing purely independent `await readFile` or similar IO operations with concurrent `Promise.all` arrays.
