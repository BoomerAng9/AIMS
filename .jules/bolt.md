...
## 2024-05-19 - N+1 Prisma Query Bottlenecks in Aggregations
**Learning:** Sequential `await prisma.<model>.findFirst` calls inside `for...of` loops iterating over aggregated results (e.g., `groupBy`) are common N+1 query bottlenecks that severely degrade API performance.
**Action:** Always refactor sequential, independent Prisma queries inside loops to execute concurrently using `Array.prototype.map` combined with `Promise.all`.
