...
## 2024-06-26 - Parallelize CDN Deploy Filesystem and GCS calls
**Learning:** Sequential asynchronous network requests (GCS) and file system writes (Nginx static dir) inside `for...of` loops are a hidden bottleneck for deployment latency, especially when deploying multiple files for a generated static site in `cdn-deploy.ts`. Refactoring these loops to use `Promise.all` with `.map()` cuts down total execution time significantly by allowing concurrent requests and I/O operations.
**Action:** Always scan for `for...of` loops that purely execute independent `await fetch` or `await fs.writeFile` calls and refactor them to use `Promise.all` mapping to reduce latency.
