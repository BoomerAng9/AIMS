...
## 2024-05-25 - [Parallelize network calls in map/reduce loops]\n**Learning:** Sequential asynchronous network requests inside `for...of` loops act as hidden latency bottlenecks. Replacing these with `Promise.all` allows parallel network requests. \n**Action:** Use `.map()` with `Promise.all` to resolve all network requests concurrently in places where order is not required and requests are independent, such as with brave search queries.
