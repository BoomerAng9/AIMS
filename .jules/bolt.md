...
## 2024-06-24 - [Parallelize DB query via Prisma in Perform State Boards Route]
**Learning:** Sequential `await prisma.<model>.findFirst(...)` calls inside loops create serious N+1 latency bottlenecks.
**Action:** Refactor loops performing sequential queries into batched chunk executions utilizing `Promise.all`. Bound chunks to a size of 10 to avoid Prisma connection pool exhaustion.
