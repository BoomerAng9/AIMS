...
## 2025-02-28 - Prisma aggregation vs findMany\n**Learning:** Using Prisma's `findMany` to pull an entire collection of database rows into memory just to calculate counts and sums creates an O(n) memory and network bottleneck. \n**Action:** Use Prisma's `aggregate` and `count` methods concurrently via `Promise.all` to push these computations to the database, converting the operation to O(1) in terms of application memory and significantly reducing latency.
